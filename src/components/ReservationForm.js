...
const ReservationForm = ({ car, user, UpdateReservations }) => {
  ...
  const handleReservationDetails = (e) => {
    ...
    fetch('https://reserveit-api.herokuapp.com/reservations', {
      method: 'POST',
      headers: {
      ...
      <div>
        <form onSubmit={handleReservationDetails} className="w-full -mt-3">
          ...
          <div>
            ...
            <textarea name="agreement" required onChange={(e) => setAgreement(e.target.value)} maxLength="100" className="appearance-none block w-full bg-gray-200 text-gray-700 rounded p-2 mb-3 leading-tight border focus:border-red-500 focus:bg-white focus:outline-none" />
